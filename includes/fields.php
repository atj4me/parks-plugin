<?php
/**
 * Adds a meta box for park details to the 'parks' post type.
 *
 * This function registers a meta box with a unique ID 'park_details' and the title 'Park Details'.
 * The content of the meta box is generated by the 'park_details_html' callback function.
 *
 * @return void
 */
...
/**
 * Renders the HTML form for park details in the WordPress admin.
 * 
 * This function creates a form table with fields for park information including:
 * - Park name (required)
 * - Location (required)
 * - Weekday hours
 * - Weekend hours
 * - Short description
 *
 * The function retrieves existing values from post meta and displays them in the form.
 * It also includes a nonce field for security purposes.
 *
 * @param WP_Post $post The post object for the current park being edited
...
/**
 * Save park meta data when a park post is saved.
 *
 * This function handles the saving of custom meta data for park posts.
 * It performs security checks including nonce verification and user capability validation
 * before saving the meta data. The function processes multiple fields including park name,
 * location, operating hours, and description.
 *
 * @param int $post_id The ID of the post being saved
 * @return void
 *
 * Security checks:
 * - Prevents saving during autosave
 * - Validates nonce
 * - Verifies user capabilities
 *
 * Processed fields:
 * - Park name (_park_name)
 * - Park location (_park_location)
 * - Weekday hours (_park_weekday_hours)
 * - Weekend hours (_park_weekend_hours)
 * - Park description (_park_description)
 *
 * @since 1.0.0
 */
...
/**
 * Enqueues JavaScript file for the parks post type.
 * 
 * This function checks if the current post type is 'parks' and if so,
 * enqueues the title.js script to be loaded in the footer.
 * The script is loaded from the plugin's assets/js directory.
 * 
 * @global string $post_type WordPress global post type variable
 * @since 1.0.0
 * @return void
 */
